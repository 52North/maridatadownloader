...
def enrich_trajectory_with_era5_data_igor(file, output_file, username, password, parameters=None, 
    ...
    era5_trajectory = era5.download(parameters=parameters, settings={}, sel_dict=sel_dict, interpolate=True,
                                    method=method_interp, interolate=True)
    
    if has_nan(era5_trajectory):
        era5_trajectory = fill_nan(era5_trajectory, method=method_extrap)
    
    # Interpolate values 
    era5_trajectory = era5_trajectory.interp(**sel_dict, method=method_interp)
